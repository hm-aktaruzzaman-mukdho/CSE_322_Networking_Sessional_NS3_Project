...
namespace ns3
{
  ...
  namespace aodv
  {
    ...
    //my_new_modification_code

    /**
     * \ingroup aodv
     * \brief Tag used to track verification probe responses in AODV wormhole detection
     * 
     * This tag is attached to verification test packets sent during the second-stage
     * RTT-based wormhole detection process. It distinguishes between initial probe
     * packets and their echo responses, allowing nodes to measure round-trip times
     * and detect potential wormhole attacks.
     */
    class VerificationProbeTag : public Tag
    {
    public:
      /**
       * \brief Constructor
       * \param isResponse indicates if this is a response packet (true) or initial probe (false)
       */
      VerificationProbeTag(bool isResponse = false) : Tag(),
                               m_isResponse(isResponse)
      {
      }

      /**
       * \brief Get the type ID for this tag class
       * \return the TypeId object used for ns-3 object serialization
       */
      static TypeId GetTypeId()
      {
      static TypeId tid = TypeId("ns3::aodv::VerificationProbeTag")
                  .SetParent<Tag>()
                  .SetGroupName("Aodv")
                  .AddConstructor<VerificationProbeTag>();
      return tid;
      }

      /**
       * \brief Get the instance type ID
       * \return the TypeId for this specific tag instance
       */
      ...
      /**
       * \brief Get the serialized size of this tag
       * \return size in bytes (1 byte for boolean flag)
       */
      ...
      /**
       * \brief Serialize the tag to a buffer for transmission
       * \param i the tag buffer to write to
       */
      void Serialize(TagBuffer i) const override
      {
      i.WriteU8(m_isResponse ? 1 : 0);
      }

      /**
       * \brief Deserialize the tag from a received buffer
       * \param i the tag buffer to read from
       */
      void Deserialize(TagBuffer i) override
      {
      m_isResponse = (i.ReadU8() != 0);
      }

      /**
       * \brief Print tag information for debugging
       * \param os the output stream
       */
      void Print(std::ostream &os) const override
      {
      os << "VerificationProbeTag: response=" << m_isResponse;
      }

      /**
       * \brief Check if this tag represents a response packet
       * \return true if this is a response, false if it's an initial probe
       */
      ...
      /**
       * \brief Set whether this tag represents a response packet
       * \param isResponse true to mark as response, false as initial probe
       */
    ...
    private:
      /// Flag indicating if packet is a probe response (true) or initial probe (false)
      bool m_isResponse;
    };

    /// Register the VerificationProbeTag with ns-3 object system
    NS_OBJECT_ENSURE_REGISTERED(VerificationProbeTag);
    //my_new_modification_code
    ...
    RoutingProtocol::RoutingProtocol()
        : m_rreqRetries(2),
          ...
          m_enableHello(false),
          //Wormhole detection
          m_enableRttWormholeDetect(true),
          m_enableSecondProbeVerification(true),
          m_wormholeMu(2.5),
          m_wormholePd(MilliSeconds(1)),
          m_wormholeCtrlPacketBytes(64),
          m_wormholeLinkRateBps(11000000.0),
          m_wormholeCpuProcessBase(MilliSeconds(1)),
          //Wormhole detection
    ...
    RoutingProtocol::GetTypeId(void)
    {
      static TypeId tid = TypeId("ns3::aodv::RoutingProtocol")
                              ...
                              .AddAttribute("SecondEndWifiWormTunnel",
                                            ...
                                            MakeIpv4AddressChecker())
                              //Wormhole detection
                              .AddAttribute("EnableRttWormholeDetect",
                                            "Enable online RTT-based wormhole detection during route discovery.",
                                            BooleanValue(true),
                                            MakeBooleanAccessor(&RoutingProtocol::SetRttWormholeDetectEnable,
                                                                &RoutingProtocol::GetRttWormholeDetectEnable),
                                            MakeBooleanChecker())
                              .AddAttribute("EnableSecondProbeVerification",
                                            "Enable second RTT probe verification before confirming wormhole suspicion.",
                                            BooleanValue(true),
                                            MakeBooleanAccessor(&RoutingProtocol::SetSecondProbeVerificationEnable,
                                                                &RoutingProtocol::GetSecondProbeVerificationEnable),
                                            MakeBooleanChecker())
                              .AddAttribute("WormholeMu",
                                            "Mu threshold multiplier for |A-E| <= mu*variation check.",
                                            DoubleValue(2.5),
                                            MakeDoubleAccessor(&RoutingProtocol::m_wormholeMu),
                                            MakeDoubleChecker<double>(0.1, 20.0))
                              .AddAttribute("WormholePd",
                                            "Propagation delay term (PD) used by RTT detector.",
                                            TimeValue(MilliSeconds(1)),
                                            MakeTimeAccessor(&RoutingProtocol::m_wormholePd),
                                            MakeTimeChecker())
                              .AddAttribute("WormholeCtrlPacketBytes",
                                            "Control packet byte size used for TT estimation.",
                                            UintegerValue(64),
                                            MakeUintegerAccessor(&RoutingProtocol::m_wormholeCtrlPacketBytes),
                                            MakeUintegerChecker<uint32_t>(1))
                              .AddAttribute("WormholeLinkRateBps",
                                            "Link rate used for TT estimation in bits per second.",
                                            DoubleValue(11000000.0),
                                            MakeDoubleAccessor(&RoutingProtocol::m_wormholeLinkRateBps),
                                            MakeDoubleChecker<double>(1.0))
                              .AddAttribute("WormholeCpuProcessBase",
                                            "Base processing time used for PT estimation.",
                                            TimeValue(MilliSeconds(1)),
                                            MakeTimeAccessor(&RoutingProtocol::m_wormholeCpuProcessBase),
                                            MakeTimeChecker());
      //Wormhole detection
      ;
      return tid;
    ...
    RoutingProtocol::SendRequest(Ipv4Address dst)
    {
           ...
           j != m_socketAddresses.end(); ++j)
      {
        ...
        //Wormhole detection
        if (m_enableRttWormholeDetect)
        {
          StartRttProbe(iface.GetLocal(), dst);
        }
        //Wormhole detection
    ...
    //Wormhole detection
    // Start RTT probe measurement for wormhole detection
    // Records the start time of the probe to measure round-trip time later
    void
    RoutingProtocol::StartRttProbe(Ipv4Address origin, Ipv4Address dst)
    {
      WormholeProbeKey key;
      key.origin = origin;
      key.dst = dst;
      m_rttProbeStartTime[key] = Simulator::Now(); // Record probe start time
    }

    // Estimate processing delay based on current buffer load
    // Higher buffer load increases the estimated processing time
    Time
    RoutingProtocol::EstimateProcessingTime(uint32_t bufferLoad) const
    {
      if (m_maxQueueLen == 0)
      {
      return m_wormholeCpuProcessBase;
      }

      // Calculate ratio of current buffer load to max capacity
      double ratio = std::min(1.0, static_cast<double>(bufferLoad) / static_cast<double>(m_maxQueueLen));
      // Scale processing time proportionally to buffer load
      double ptSec = m_wormholeCpuProcessBase.GetSeconds() * (1.0 + ratio);
      return Seconds(ptSec);
    }

    // Estimate transmission delay for a control packet based on link rate
    // Calculates time = (packet_size * 8 bits) / link_rate_bps
    Time
    RoutingProtocol::EstimateTransmissionTime(uint32_t packetBytes) const
    {
      double bits = static_cast<double>(packetBytes) * 8.0;
      double ttSec = bits / std::max(1.0, m_wormholeLinkRateBps);
      return Seconds(ttSec);
    }

    // Update exponential moving average statistics for neighbor RTT measurements
    // Maintains mean and variance of round-trip times for wormhole detection thresholds
    void
    RoutingProtocol::UpdateNeighborRttStats(Ipv4Address neighbor, Time sample)
    {
      std::map<Ipv4Address, Time>::iterator meanIt = m_neighborEstimatedRtt.find(neighbor);
      std::map<Ipv4Address, Time>::iterator varIt = m_neighborEstimatedVar.find(neighbor);

      // Initialize statistics on first sample
      if (meanIt == m_neighborEstimatedRtt.end())
      {
      m_neighborEstimatedRtt[neighbor] = sample;
      m_neighborEstimatedVar[neighbor] = sample / 2;
      return;
      }

      // Exponential moving average constants for mean and variance
      const double alpha = 0.125;
      const double beta = 0.25;
      Time currentMean = meanIt->second;
      Time currentVar = (varIt == m_neighborEstimatedVar.end()) ? (sample / 2) : varIt->second;

      // Calculate error between sample and current mean
      Time err = sample - currentMean;
      Time absErr = (err.GetNanoSeconds() < 0) ? (Seconds(0) - err) : err;
      // Update mean with exponential smoothing
      Time newMean = currentMean + Time::FromDouble(alpha * err.GetSeconds(), Time::S);
      // Update variance based on absolute error deviation
      Time varDelta = absErr - currentVar;
      Time newVar = currentVar + Time::FromDouble(beta * varDelta.GetSeconds(), Time::S);

      m_neighborEstimatedRtt[neighbor] = newMean;
      m_neighborEstimatedVar[neighbor] = newVar;
    }

    // Check if a neighbor is currently under temporary suspicion for wormhole attack
    // Returns true if current time is within the suspicion window
    bool
    RoutingProtocol::IsNeighborTemporarilySuspect(Ipv4Address neighbor) const
    {
      std::map<Ipv4Address, Time>::const_iterator it = m_suspectUntil.find(neighbor);
      if (it == m_suspectUntil.end())
      {
      return false;
      }
      return Simulator::Now() <= it->second; // Still within blacklist timeout
    }

    // Evaluate first-stage RTT probe response for wormhole detection
    // Compares actual RTT against expected baseline to detect anomalies
    // If anomaly detected and verification enabled, triggers second probe stage
    bool
    RoutingProtocol::EvaluateRttProbe(Ipv4Address origin, Ipv4Address dst, Ipv4Address sender, Ipv4Address receiver, Time actualRtt) //my_new_modification_code
    {
      WormholeProbeKey key;
      key.origin = origin;
      key.dst = dst;

      // Find matching probe start time for this origin-destination pair
      std::map<WormholeProbeKey, Time>::iterator probeIt = m_rttProbeStartTime.find(key);
      if (probeIt == m_rttProbeStartTime.end())
      {
      return false; // No matching probe found
      }

      // Estimate delays: processing time, transmission time, and propagation delay
      uint32_t bufferLoad = static_cast<uint32_t>(m_rttProbeStartTime.size());
      Time pt = EstimateProcessingTime(bufferLoad);
      Time tt = EstimateTransmissionTime(m_wormholeCtrlPacketBytes);

      // Use neighbor's historical RTT as baseline, or default to 2*NodeTraversalTime
      Time baseline = 2 * m_nodeTraversalTime;
      std::map<Ipv4Address, Time>::const_iterator meanIt = m_neighborEstimatedRtt.find(sender);
      if (meanIt != m_neighborEstimatedRtt.end())
      {
      baseline = meanIt->second;
      }

      // Calculate expected RTT and compare against actual measurement
      Time expected = baseline + pt + tt + m_wormholePd;
      Time diff = actualRtt - expected;
      Time absDiff = (diff.GetNanoSeconds() < 0) ? (Seconds(0)-diff) : diff;

      // Determine detection threshold based on learned variance
      Time variation = MilliSeconds(1);
      std::map<Ipv4Address, Time>::const_iterator varIt = m_neighborEstimatedVar.find(sender);
      if (varIt != m_neighborEstimatedVar.end() && varIt->second > MicroSeconds(1))
      {
      variation = varIt->second;
      }

      // Wormhole detected if absolute difference exceeds mu * variance threshold
      Time threshold = Time::FromDouble(m_wormholeMu * variation.GetSeconds(), Time::S);
      bool detected = absDiff > threshold;

      if (detected)
      {
      //my_new_modification_code
      // If second probe verification enabled, don't immediately blacklist
      // Instead, send verification probe and wait for response
      if (m_enableSecondProbeVerification)
      {
        std::map<Ipv4Address, Time>::iterator verifyIt = m_underVerification.find(sender);
        if (verifyIt == m_underVerification.end())
        {
        m_underVerification[sender] = Simulator::Now(); // Mark neighbor as under verification
        NS_LOG_WARN("RTT anomaly suspected (entering verification). node=" << m_ipv4->GetObject<Node>()->GetId()
                                           << " sender=" << sender
                                           << " actual=" << actualRtt
                                           << " expected=" << expected
                                           << " absDiff=" << absDiff
                                           << " threshold=" << threshold);
        SendVerificationTestPacket(receiver, sender); // Send second-stage probe
        }
        m_rttProbeStartTime.erase(probeIt);
        return false; // Defer final detection decision
      }
      // If verification disabled, immediately mark neighbor as malicious
      m_suspectUntil[sender] = Simulator::Now() + m_blackListTimeout;
      m_detectedWormholePeers.insert(sender);
      //my_new_modification_code
      }
      else
      {
      // No anomaly detected, update neighbor RTT statistics for future comparisons
      UpdateNeighborRttStats(sender, actualRtt);
      }

      m_rttProbeStartTime.erase(probeIt); // Clean up probe tracking
      return detected;
    }
    //Wormhole detection
    ...
    //my_new_modification_code
    void
    RoutingProtocol::SendVerificationTestPacket(Ipv4Address origin, Ipv4Address neighbor)//my_new_modification_code
    {
      WormholeProbeKey verifyKey;
      verifyKey.origin = origin; //my_new_modification_code
      verifyKey.dst = neighbor;

      m_verificationProbeStartTime[verifyKey] = Simulator::Now();
//my_new_modification_code
      Ptr<Socket> socket = FindSocketWithInterfaceAddress(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(origin), 0));
      NS_ASSERT(socket);

      Ptr<Packet> packet = Create<Packet>();
      VerificationProbeTag verifyTag(false);
      packet->AddPacketTag(verifyTag);
      SocketIpTtlTag ttlTag;
      ttlTag.SetTtl(1);
      packet->AddPacketTag(ttlTag);
      packet->AddHeader(TypeHeader(AODVTYPE_RVERIFY));

      NS_LOG_DEBUG("Sending verification test packet to neighbor: " << neighbor
                   << " from node " << m_ipv4->GetObject<Node>()->GetId()
                   << " on interface " << origin);
    ...
    bool
    RoutingProtocol::EvaluateVerificationProbe(Ipv4Address origin, Ipv4Address neighbor, Time verificationRtt)//my_new_modification_code
    {
      WormholeProbeKey verifyKey;
      verifyKey.origin = origin;//my_new_modification_code
      verifyKey.dst = neighbor;
      
      std::map<WormholeProbeKey, Time>::iterator verifyProbeIt = m_verificationProbeStartTime.find(verifyKey);
      if (verifyProbeIt == m_verificationProbeStartTime.end())
      {
        return false;
      }
      
      uint32_t bufferLoad = static_cast<uint32_t>(m_verificationProbeStartTime.size());//my_new_modification_code
      Time pt = EstimateProcessingTime(bufferLoad);
      Time tt = EstimateTransmissionTime(m_wormholeCtrlPacketBytes);
      
      Time baseline = 2 * m_nodeTraversalTime;
      std::map<Ipv4Address, Time>::const_iterator meanIt = m_neighborEstimatedRtt.find(neighbor);
      if (meanIt != m_neighborEstimatedRtt.end())
      {
        baseline = meanIt->second;
      }
      ...
      Time expected = baseline + pt + tt + m_wormholePd;
      Time diff = verificationRtt - expected;
      Time absDiff = (diff.GetNanoSeconds() < 0) ? (Seconds(0)-diff) : diff;
      
      Time variation = MilliSeconds(1);
      std::map<Ipv4Address, Time>::const_iterator varIt = m_neighborEstimatedVar.find(neighbor);
      if (varIt != m_neighborEstimatedVar.end() && varIt->second > MicroSeconds(1))
      {
        variation = varIt->second;
      }
      ...
      Time threshold = Time::FromDouble(m_wormholeMu * variation.GetSeconds(), Time::S);
      bool verificationDetected = absDiff > threshold;
      
      if (verificationDetected)
      {
        m_verificationFailureCount[neighbor]++;
        m_suspectUntil[neighbor] = Simulator::Now() + m_blackListTimeout;
        m_detectedWormholePeers.insert(neighbor);
        NS_LOG_WARN("RTT wormhole CONFIRMED by verification. node=" << m_ipv4->GetObject<Node>()->GetId()
                                                                     << " neighbor=" << neighbor
                                                                     << " verificationRtt=" << verificationRtt
                                                                     << " expected=" << expected
                                                                     << " absDiff=" << absDiff
                                                                     << " threshold=" << threshold
                                                                     << " verificationCount=" << m_verificationFailureCount[neighbor]);//my_new_modification_code
      }
      else
      {
        m_verificationFailureCount.erase(neighbor);
        UpdateNeighborRttStats(neighbor, verificationRtt);
        NS_LOG_WARN("RTT verification CLEARED (temporary slowdown). node=" << m_ipv4->GetObject<Node>()->GetId()
                                                                           << " neighbor=" << neighbor
                                                                           << " verificationRtt=" << verificationRtt
                                                                           << " expected=" << expected
                                                                           << " absDiff=" << absDiff
      ...
      m_underVerification.erase(neighbor);
      m_verificationProbeStartTime.erase(verifyProbeIt);
    ...
    RoutingProtocol::RecvAodv(Ptr<Socket> socket)
    {
      ...
      //my_new_modification_code
      if (tHeader.Get() == AODVTYPE_RVERIFY)
      {
        if (!m_enableSecondProbeVerification)
        {
          NS_LOG_DEBUG("Second probe verification disabled. Dropping verification packet.");
          return;
        }

        VerificationProbeTag verifyTag;
        if (!packet->PeekPacketTag(verifyTag))
        {
          NS_LOG_DEBUG("Verification packet without tag received. Drop");
          return;
        }

        packet->RemovePacketTag(verifyTag);
        if (!verifyTag.IsResponse())
        {
          verifyTag.SetResponse(true);
          packet->AddPacketTag(verifyTag);
          packet->AddHeader(TypeHeader(AODVTYPE_RVERIFY));
          NS_LOG_DEBUG("Echoing verification probe from " << sender << " back to origin " << receiver);
          socket->SendTo(packet, 0, InetSocketAddress(sender, AODV_PORT));
          return;
        }

        std::map<Ipv4Address, Time>::const_iterator verifyIt = m_underVerification.find(sender);
        if (verifyIt == m_underVerification.end())
        {
          NS_LOG_DEBUG("Verification response from " << sender << " arrived with no pending suspicion");
          return;
        }

        WormholeProbeKey verifyKey;
        verifyKey.origin = receiver;
        verifyKey.dst = sender;
        std::map<WormholeProbeKey, Time>::const_iterator verifyProbeIt = m_verificationProbeStartTime.find(verifyKey);
        if (verifyProbeIt == m_verificationProbeStartTime.end())
        {
          NS_LOG_DEBUG("Verification response from " << sender << " had no matching start time");
          return;
        }

        Time verificationRtt = Simulator::Now() - verifyProbeIt->second;
        if (EvaluateVerificationProbe(receiver, sender, verificationRtt))
        {
          NS_LOG_WARN("RTT verification confirmed wormhole. Rejecting verification reply from neighbor " << sender);
        }
        else
        {
          NS_LOG_WARN("RTT verification cleared suspicion on neighbor " << sender << ".");
        }
        return;
      }
      //my_new_modification_code
    ...
    RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
    {
      ...
      if (m_enableRttWormholeDetect)
      {
        if (IsNeighborTemporarilySuspect(sender))
        {
          NS_LOG_WARN("Dropping RREP from temporarily suspect neighbor " << sender);
          return;
        }

        WormholeProbeKey key;
        key.origin = rrepHeader.GetOrigin();
        key.dst = rrepHeader.GetDst();
        std::map<WormholeProbeKey, Time>::const_iterator probeIt = m_rttProbeStartTime.find(key);
        if (probeIt != m_rttProbeStartTime.end())
        {
          Time actualRtt = Simulator::Now() - probeIt->second;
          if (EvaluateRttProbe(rrepHeader.GetOrigin(), rrepHeader.GetDst(), sender, receiver, actualRtt)) //my_new_modification_code
          {
            NS_LOG_WARN("RTT detector rejected RREP for origin=" << rrepHeader.GetOrigin() << " dst=" << rrepHeader.GetDst());
            return;
          }
        }
      }
