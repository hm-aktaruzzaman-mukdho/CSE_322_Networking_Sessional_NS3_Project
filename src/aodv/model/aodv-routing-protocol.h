...
namespace ns3
{
  ...
  namespace aodv
  {
    ...
    public:
      ...
      //Wormhole detection
      void SetRttWormholeDetectEnable(bool f) { m_enableRttWormholeDetect = f; }
      bool GetRttWormholeDetectEnable() const { return m_enableRttWormholeDetect; }
      void SetSecondProbeVerificationEnable(bool f) { m_enableSecondProbeVerification = f; }
      bool GetSecondProbeVerificationEnable() const { return m_enableSecondProbeVerification; }
      //Wormhole detection
    ...
    private:
      ...
      //Wormhole detection
      struct WormholeProbeKey
      {
        Ipv4Address origin;
        Ipv4Address dst;

        bool operator<(const WormholeProbeKey &other) const
        {
          if (origin.Get() != other.origin.Get())
          {
            return origin.Get() < other.origin.Get();
          }
          return dst.Get() < other.dst.Get();
        }
      };

      bool m_enableRttWormholeDetect;
      bool m_enableSecondProbeVerification;
      double m_wormholeMu;
      Time m_wormholePd;
      uint32_t m_wormholeCtrlPacketBytes;
      double m_wormholeLinkRateBps;
      Time m_wormholeCpuProcessBase;
      std::map<WormholeProbeKey, Time> m_rttProbeStartTime;
      std::map<Ipv4Address, Time> m_neighborEstimatedRtt;
      std::map<Ipv4Address, Time> m_neighborEstimatedVar;
      std::map<Ipv4Address, Time> m_suspectUntil;
      std::set<Ipv4Address> m_detectedWormholePeers;
      //Wormhole detection

      //my_new_modification_code
      std::map<Ipv4Address, Time> m_underVerification;
      std::map<WormholeProbeKey, Time> m_verificationProbeStartTime;
      std::map<Ipv4Address, uint32_t> m_verificationFailureCount;
      //my_new_modification_code
    ...
    private:
      ...
      //Wormhole detection
      void StartRttProbe(Ipv4Address origin, Ipv4Address dst);
      //my_new_modification_code
      bool EvaluateRttProbe(Ipv4Address origin, Ipv4Address dst, Ipv4Address sender, Ipv4Address receiver, Time actualRtt);
      //my_new_modification_code
      Time EstimateProcessingTime(uint32_t bufferLoad) const;
      Time EstimateTransmissionTime(uint32_t packetBytes) const;
      void UpdateNeighborRttStats(Ipv4Address neighbor, Time sample);
      bool IsNeighborTemporarilySuspect(Ipv4Address neighbor) const;
      //Wormhole detection

      //my_new_modification_code
      void SendVerificationTestPacket(Ipv4Address origin, Ipv4Address neighbor);
      bool EvaluateVerificationProbe(Ipv4Address origin, Ipv4Address neighbor, Time verificationRtt);
      //my_new_modification_code
